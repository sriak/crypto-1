...
func TestSignature2Read(t *testing.T) {
	packet, err := Read(readerFromHex(signature2DataHex))
	if err != nil {
		t.Error(err)
		return
	}
	sig, ok := packet.(*Signature)
	if !ok || sig.SigType != SigTypeBinary || sig.PubKeyAlgo != PubKeyAlgoEdDSA || sig.Hash != crypto.SHA256 {
		t.Errorf("failed to parse, got: %#v", packet)
	}
}
...
const signature2DataHex= "c25e040016080010050258314c4e0910941156fe2376e3590000adf201006b49fbda969170dca6278af97f4f31e4d56f74a11896fbbc6782dad2a8e5fd6501005958987deba9c5583a39727e05016233cb28d94bfc7856fdd2700b318e2e630d"